...
public class RadarCRC16 {
    ...
    public static int crc16BaseRadar(byte[] dataIn) {
        ...
        int tableNo;
        for (byte b : dataIn) {
            tableNo = ((result & 0xff) ^ (b & 0xff));
            result = ((result >> 8) & 0xff) ^ LOOKUP_TABLE[tableNo];
        }
        return result;
    }

    public static int crc16BaseRadar(byte[] dataIn, int offset, int length) {
        if (offset < 0 || offset + length > dataIn.length) {
            throw new IllegalArgumentException("please check the argument [offset,length]");
        }
